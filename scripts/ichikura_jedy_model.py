...
#     target_coords=Coordinates(pos=[0.13, -0.5, 0.1]),
#     rotation_axis=False)

copy_coords = robot_model.larm.end_coords.translation

robot_model.larm.inverse_kinematics(
    target_coords=Coordinates(pos=[-0.117,0.090, 0.061]),
    rotation_axis=False,
    maxiter=1000)

success = robot_model.larm.inverse_kinematics(
    target_coords=Coordinates(pos=[-0.117,0.090, 0.061]),
    rotation_axis=False,
    maxiter=1000)
...
if success is not False and success is not None and isinstance(success, np.ndarray):  
    joint_angle = robot_model.angle_vector()
    print("Updated joint angle:", joint_angle)
    joint_angle[:6] = robot_model.larm.angle_vector() 
    ri.angle_vector(joint_angle, 3, controller_type="larm_controller")
    print(ri.angle_vector())
    ri.wait_interpolation()
    time.sleep(3)
else:
